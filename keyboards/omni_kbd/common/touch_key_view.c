...
enum {
    TOUCH_KEY_LAYER_COUNT    = MAX_LCD_LAYER + 1,
    TOUCH_KEY_CATEGORY_COUNT = MAX_LCD_CATEGORY + 1,
    TOUCH_KEYS_PER_LAYER     = 6,
    TOUCH_KEY_IMAGE_COUNT    = TOUCH_KEYS_PER_LAYER + 1,
    TOUCH_KEY_RADIUS         = 30,
...
static const touch_key_position_t key_centers[TOUCH_KEYS_PER_LAYER] = {
    {.x = 200, .y = 120},
    {.x = 160, .y = 189},
    {.x = 80,  .y = 189},
    {.x = 40,  .y = 120},
...
static touch_key_image_t layer_images[TOUCH_KEY_CATEGORY_COUNT][TOUCH_KEY_LAYER_COUNT][TOUCH_KEY_IMAGE_COUNT];
static uint8_t current_layer;
static uint8_t current_category;

void touch_key_view_initialize(const uint16_t *virtual_keycodes) {
    for (int layer_index = 0; layer_index < TOUCH_KEY_LAYER_COUNT; layer_index++) {
        for (int category_index = 0; category_index < TOUCH_KEY_CATEGORY_COUNT; category_index++) {
            int image_index = category_index + layer_index * TOUCH_KEY_CATEGORY_COUNT;
            layer_images[category_index][layer_index][0] = (touch_key_image_t){
                .image  = omni_layer_image_handle(image_index),
                .center = layer_image_center,
            };

            for (int key_index = 0; key_index < TOUCH_KEYS_PER_LAYER; key_index++) {
                int keycode_index = key_index + category_index * TOUCH_KEYS_PER_LAYER + layer_index * TOUCH_KEY_CATEGORY_COUNT * TOUCH_KEYS_PER_LAYER;
                layer_images[category_index][layer_index][key_index + 1] = (touch_key_image_t){
...
void touch_key_view_draw(painter_device_t display) {
    touch_key_image_t *current_images = layer_images[current_category][current_layer];
...
void touch_key_view_handle_gesture(painter_device_t display, uint8_t gesture_id) {
    switch (gesture_id) {
        case CST816S_SLIDE_RIGHT:
            if (current_layer < MAX_LCD_LAYER) {
                current_layer++;
        ...
        case CST816S_SLIDE_DOWN:
            if (current_category < MAX_LCD_CATEGORY) {
                current_category++;
                clear_and_draw(display);
            }
            break;
...
static uint8_t find_touched_key(uint16_t touch_x, uint16_t touch_y) {
    for (uint8_t key_index = 0; key_index < TOUCH_KEYS_PER_LAYER; key_index++) {
...
bool touch_key_view_locate_key(uint16_t touch_x, uint16_t touch_y, uint8_t *virtual_row, uint8_t *virtual_column) {
    ...
    *virtual_row = current_category + current_layer * TOUCH_KEY_CATEGORY_COUNT;
